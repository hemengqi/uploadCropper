	...
	var crop = function(obj,args){
		...
		var cropper = {
			...
			updateCoords: function(c){
				...
				if(parseInt(c.w)>0){
					...
					if(ph<=pw){
						...
						$preview.css({
							width: Math.round(pw*whR) + 'px',
							height: pw,
						});
						zR = pw/c.h;
					};
					$previewImg.css({
						width: Math.round(zR * $image['o_width']) + 'px',
			          	//height: Math.round(zR * $image['o_height']) + 'px',
			          	marginLeft: '-' + Math.round(zR * c.x) + 'px',
			          	marginTop: '-' + Math.round(zR * c.y) + 'px'
					});
				}
			},
			//清空裁剪数据
			clearCoords:function(){
				...
				$('#cropInfo input,#cropAction input').val('');
				    ...
				    if($cropAction){
					    $cropAction.on('click','.boxRatio',function(){
					    	...
					    	if(c_w){
						    	...
						    	$image['w'] = c_w;
						    	$image['h'] = $image['h']?$image['h']:100;
						    	jcrop_api.animateTo([$image['x'],$image['y'],$image['x']+$image['w'],$image['y']+$image['h']]);
					    	}
					    }).on('click','#customH-btn',function(){
					    	//固定图片宽度
					    	var c_h = parseInt($('#customH').val());
					    	if(c_h){
						    	jcrop_api.setOptions({
						    		aspectRatio: 0,
						    		maxSize: args.maxSize,
